...
private:
    ...
    vector <pair<double, vector<Point>>> solutions;
    double* assignment;
...
public:
    ...
    void set_solution(vector <pair<double, vector<Point>>> s) { solutions = s; }
    vector <pair<double, vector<Point>>> get_solution() { return solutions; }
...
bool check_conflict(Node* node, int numofagents, tuple<int, int, Point, int> &constraint) {
    vector <pair<double, vector<Point>>> sol = node->get_solution();
    for (int i = 0; i < numofagents; i++)
    {
        for (int j = i+1; j < numofagents - 1; j++) {
            int m;
            vector<int> agent1 = sol[i].second;
            vector<int> agent2 = sol[j].second;
            ...
            for (int k = 0; k < m; k++) {
                if (agent1[k] == agent2[k]) {
                    constraint = make_tuple(i, j, agent1[k], k);
                    return 0;
                }
...
double get_SIC(Node* node, int numofagents) {
    double cost = 0;
    vector <pair<double, vector<Point>>> solutions = node->get_solution();
    for (int i = 0; i < numofagents; i++)
    {
        cost += solutions[i].first;
    }
    return cost;
}
...
{   
    ...
    if (curr_time == 0) {
        ...
        while (!OPEN.empty()) {
            ...
            child_node2->set_solution(unconstrainedSearch(gridmap, starts, goals_child, child_node2->get_constraints()));
    ...
    vector <pair<double, vector<Point>>> set_of_sol = final_node->get_solution();
    if (goals_reached) {
        for (int i = 0; i < numofagents; i++) {
            vector<Point> sol = set_of_sol[i].second;
